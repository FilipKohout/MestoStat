    ...
    return (
        <nav className="sticky top-0 z-50 w-full border-b border-slate-800 bg-slate-950/80 backdrop-blur-md">
            <div className="mx-auto flex h-14 max-w-7xl items-center justify-between px-4 sm:px-6 gap-4">
                ...
                {/* 2. SEARCHBAR (Uprostřed, roztahuje se) */}
                <div className="hidden sm:flex flex-1 max-w-md mx-auto">
                    <div className="relative w-full group">
                        ...
                        <input
                            ...
                            placeholder="Hledat obec, IČO..."
                        />
                    </div>
                </div>

                {/* 3. ODRAZKY (Odkaz "Hledat" jsem smazal, protože máme input) */}
                <div className="flex items-center gap-6 text-sm text-slate-400 font-medium shrink-0">
                    ...
                    {/* Avatar / Profil (jen jako placeholder pro profi look) */}
                    <div className="h-7 w-7 rounded-full bg-slate-800 border border-slate-700 hover:border-slate-500 transition-colors cursor-pointer" />
                </div>
            </div>
        </nav>
    );
